...
import de.swiftbyte.gmc.common.packet.from.backend.server.ServerDeleteBackupPacket;
...
public class DeleteBackupServerPacketConsumer implements StompPacketConsumer<ServerDeleteBackupPacket> {
    ...
    @Override
    public void onReceive(@NonNull ServerDeleteBackupPacket packet) {
        log.info("Delete backup with id {}.", packet.getBackupId());

        if (getNode().getConnectionState() != ConnectionState.CONNECTED) {
